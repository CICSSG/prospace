...
export default function Header() {
  ...
  return (
    <>
      ...
      {isMenuOpen && (
        <div className="fixed top-0 left-0 z-11 h-full w-full bg-primary p-4 lg:hidden max-w-lg">
          ...
          <Link href={"/job-fair"} className="block py-2" onClick={() => setIsMenuOpen(false)}>
            Job Fair
          </Link>
          <Link href={"/sessions"} className="block py-2" onClick={() => setIsMenuOpen(false)}>
            Sessions
          </Link>
          ...
          <div className="mt-4 flex flex-row gap-2 w-full">
            <Show when="signed-out">
              ...
              >
                <button className="h-10 cursor-pointer rounded-full bg-white px-4 text-sm font-medium text-primary sm:h-12 sm:px-5 sm:text-base w-full">
                  Sign In
                </button>   
                </SignInButton> 
              <SignUpButton
            ...
            <Show when="signed-in">
              <SignOutButton >
                <button className="h-10 cursor-pointer rounded-full bg-white px-4 text-sm font-medium text-primary sm:h-12 sm:px-5 sm:text-base w-full" onClick={() => setIsMenuOpen(false)}>
                  Sign Out
                </button>
              </SignOutButton>
            </Show>
