...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#05091d] px-4 py-6 text-white sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <section className="rounded-[2rem] border border-white/10 bg-linear-to-br from-white/10 via-white/5 to-transparent p-6 shadow-2xl">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Link
                href="/admin/users"
                className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-medium text-[#05091d] transition hover:bg-white/90"
              >
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            {statItems.map((item) => {
              ...
              return (
                <div key={item.label} className={`rounded-2xl border border-white/10 bg-linear-to-br ${item.accent} p-4`}>
                  ...
                  <p className="mt-3 text-xs leading-5 text-white/65">{item.hint}</p>
                </div>
              )
            })}
          </div>
          ...
          {[
            ...
            return (
              <Link
                key={action.href}
                href={action.href}
                className={`${statCardClass} group flex flex-col gap-4 transition hover:border-white/20 hover:bg-white/10`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-[0.25em] text-white/50">Quick add</p>
                    <h2 className="mt-2 text-xl font-semibold">{action.title}</h2>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/10 p-2 text-white/80 transition group-hover:translate-x-0.5">
                    <Icon size={18} />
                  </div>
                </div>
                <p className="text-sm leading-6 text-white/70">{action.desc}</p>
                <span className="inline-flex w-fit items-center gap-2 text-sm text-white/90">
                  Open <Plus size={14} />
                </span>
              </Link>
            )
        ...
        <section className="grid gap-4 xl:grid-cols-3">
          <div className={`${statCardClass} xl:col-span-1`}>
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.25em] text-white/50">Recent registrations</p>
                <h2 className="mt-2 text-xl font-semibold">New users</h2>
              </div>
              <Link href="/admin/users" className="text-sm text-white/70 transition hover:text-white">
                View all
              </Link>
            </div>
            ...
            <div className="mt-4 space-y-3">
              {(stats?.recentUsers ?? []).length > 0 ? (
                stats!.recentUsers.map((user) => (
                  <div key={user.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="rounded-full border border-white/10 px-2 py-1 text-[11px] uppercase tracking-[0.2em] text-white/60">
                        {user.role || "user"}
                      </span>
                    </div>
                ...
                <div className="rounded-2xl border border-dashed border-white/10 p-6 text-sm text-white/50">Loading users...</div>
              ) : (
                <div className="rounded-2xl border border-dashed border-white/10 p-6 text-sm text-white/50">No user data yet.</div>
              )}
            </div>
          </div>
          ...
          <div className={`${statCardClass} xl:col-span-1`}>
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.25em] text-white/50">Upcoming sessions</p>
                <h2 className="mt-2 text-xl font-semibold">Scheduled talks</h2>
              </div>
              <Link href="/admin/sessions" className="text-sm text-white/70 transition hover:text-white">
                View all
              </Link>
            </div>
              ...
              {(stats?.recentSessions ?? []).length > 0 ? (
                stats!.recentSessions.map((session) => (
                  <div key={session.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    ...
                    <p className="mt-2 text-xs text-white/60">{session.date || "No date"}</p>
                ...
                <div className="rounded-2xl border border-dashed border-white/10 p-6 text-sm text-white/50">Loading sessions...</div>
              ) : (
                <div className="rounded-2xl border border-dashed border-white/10 p-6 text-sm text-white/50">No session data yet.</div>
              )}
            </div>
          </div>
          ...
          <div className={`${statCardClass} xl:col-span-1`}>
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.25em] text-white/50">Recent companies</p>
                <h2 className="mt-2 text-xl font-semibold">Partners</h2>
              </div>
              <Link href="/admin/company" className="text-sm text-white/70 transition hover:text-white">
                Manage
              </Link>
            </div>
            ...
            <div className="mt-4 space-y-3">
              {(stats?.recentCompanies ?? []).length > 0 ? (
                ...
                <div className="rounded-2xl border border-dashed border-white/10 p-6 text-sm text-white/50">Loading companies...</div>
              ) : (
                <div className="rounded-2xl border border-dashed border-white/10 p-6 text-sm text-white/50">No company data yet.</div>
              )}
            </div>
          </div>
        </section>
        ...
        <section className={`${statCardClass} flex flex-col gap-4 md:flex-row md:items-center md:justify-between`}>
          <div>
            <p className="text-xs uppercase tracking-[0.25em] text-white/50">Last sync</p>
            <h2 className="mt-2 text-xl font-semibold">{stats ? formatDateTime(stats.lastUpdated) : loading ? "Loading..." : "No data"}</h2>
            <p className="mt-1 text-sm text-white/60">Refresh this dashboard whenever you add or update records.</p>
          </div>
